                ...
                <div class="col-md-12">
                    ...
                    @if($products->count() > 0)
                                ...
                                <tr>
                                    <td>
                                        <img style="width: 200px;height: 50px"
                                             src="{{ asset($product->wishlistList->first_image[0]['image']) ?? 'Null'}}"
                                             class=""
                                    ...
                                    <td>
                                        <form action="{{ route('cart.store') }}" method="POST"
                                            ...
                                            <input type="hidden" value="{{ $product->wishlistList->title }}"
                                                   name="name">
                                            <input type="hidden" value="{{ $product->wishlistList->price }}"
                                                   name="price">
                                            <input type="hidden"
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-success btn-height"
                                           href="{{ route('wishlist.remove',$product->id) }}">x</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
                        ...
                        <div class="d-flex justify-content-end py-3">
                            <a class="btn btn-sm btn-success btn-height" href="{{ route('wishlist.clear') }}">Clear
                                Wishlist</a>
                        </div>
